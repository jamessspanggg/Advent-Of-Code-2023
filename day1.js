...
function sumOfCalibratedValues(input) {
  ...
  for (let row of rows) {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      if (!isNaN(Number.parseInt(row.charAt(i))) && firstDigit == undefined) {
        firstDigit = Number.parseInt(row.charAt(i))
      }
      if (!isNaN(Number.parseInt(row.charAt(row.length - i - 1))) && lastDigit == undefined) {
        lastDigit = Number.parseInt(row.charAt(row.length - i - 1))
      }

      if (firstDigit != undefined && lastDigit != undefined) break;
    }
